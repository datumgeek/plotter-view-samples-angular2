...
export class SunburstComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.arc = d3.svg.arc()
            .startAngle(function (d: any) { return Math.max(0, Math.min(2 * Math.PI, that.x(d.x))); })
            .endAngle(function (d: any) { return Math.max(0, Math.min(2 * Math.PI, that.x(d.x + d.dx))); })
            .innerRadius(function (d: any) { return Math.max(0, that.y(d.y)); })
            .outerRadius(function (d: any) { return Math.max(0, that.y(d.y + d.dy)); });
        ...
        this.path = svg.datum(root).selectAll("path")
            ...
            .enter().append("path")
            .attr("d", that.arc)
            .style("fill", function (d: any) { return color((d.children ? d : d.parent).name); })
        ...
        d3.select(this.formElement).selectAll("input").on("change", function change() {
            ...
            that.path
                .data(that.partition.value.call(this, value).nodes)
                .transition()
                .duration(1000)
                .attrTween("d", function(d: any, i: any) {
                    that.arcTweenData.call(this, that, d, i);
                });
        });
    ...
    arcTweenData(that: SunburstComponent, a: any, i: any) {

        var oi = d3.interpolate({ x: a.x0, dx: a.dx0 }, a);
        function tween(t: any) {
            var b = oi(t);
            a.x0 = b.x;
            a.dx0 = b.dx;
    ...
    arcTweenZoom(that: SunburstComponent, d: any) {
        let myThis: any = this;
    ...
    click(that: SunburstComponent, d: any) {
        let myThat = this;
